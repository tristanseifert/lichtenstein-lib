...
//    SSL_CTX_set_verify(ctx, SSL_VERIFY_PEER | SSL_VERIFY_CLIENT_ONCE, dtls_verify_callback);
  ...
  std::shared_ptr<GenericServerClient> DTLSServer::run() {
    ...
    setsockopt(clientFd, SOL_SOCKET, SO_REUSEADDR, (const void*) &on, (socklen_t) sizeof(on));
#if defined(SO_REUSEPORT) && !defined(__linux__)
    setsockopt(clientFd, SOL_SOCKET, SO_REUSEPORT, (const void*) &on, (socklen_t) sizeof(on));
#endif

    // read the server socket's address
    socklen_t serverAddrLen = sizeof(serverAddr);

    if(getsockname(this->listeningSocket, (struct sockaddr *) &serverAddr, &serverAddrLen) == -1) {
      throw std::system_error(errno, std::system_category(),
                              "getsockname() failed");
    }
    ...
    switch (clientAddr.ss.ss_family) {
      case AF_INET:
        ...
        if(bind(clientFd, (const struct sockaddr *) &serverAddr, sizeof(struct sockaddr_in))) {
          throw std::system_error(errno, std::system_category(), "bind() on client socket");
        }
        if(connect(clientFd, (struct sockaddr *) &clientAddr, sizeof(struct sockaddr_in))) {
          throw std::system_error(errno, std::system_category(), "connect() on client socket");
        }
      ...
      case AF_INET6:
        ...
        setsockopt(clientFd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&off, sizeof(off));

        // then bind to local address, and set the remote host's address
        if(bind(clientFd, (const struct sockaddr *) &serverAddr, sizeof(struct sockaddr_in6))) {
          throw std::system_error(errno, std::system_category(), "bind() on client socket");
        }
        if(connect(clientFd, (struct sockaddr *) &clientAddr, sizeof(struct sockaddr_in6))) {
          throw std::system_error(errno, std::system_category(), "connect() on client socket");
        }
        break;

      default:
        throw std::runtime_error("Invalid address family " + std::to_string(clientAddr.ss.ss_family));
    }
    ...
    auto *client = new GenericServerClient(this, clientFd, ssl, clientAddr.s4);
  ...
  /**
   * Generates a DTLS session cookie. This contains some random bytes, plus the
   * address/port of the connecting client.
   *
  ...
  int DTLSVerifyCookieCb(SSL *ssl, unsigned char *cookie, unsigned int cookieLen) {
    ...
    if (!cookieSecretValid) {
      LOG(ERROR) << "Attempt to validate DTLS cookie (len = " << cookieLen << ") but cookie secret has not been initialized";
      return 0;
    }
    // calculate the cookie
    DTLSCalculateCookie(ssl, result, &resultLength);
    ...
    if(cookieLen == resultLength && memcmp(result, cookie, resultLength) == 0) {
      // cookie was valid
      return 1;
    }
