...
namespace liblichtenstein::io {
   ...
   */
  GenericServerClient::GenericServerClient(GenericTLSServer *_server, int _fd,
                                           SSL *_ctx,
                                           struct sockaddr_in _addr) : server(
          _server), fd(_fd), ctx(_ctx), clientAddr(_addr) {
  ...
  /**
   * Writes data to the client through the SSL session. This invokes OpenSSL
   ...
   * @return How many bytes were actually read, or -1 if session was closed
   * @throws std::system_error, TLSServer::OpenSSLError
   */
  size_t
  GenericServerClient::read(std::vector<std::byte> &data, size_t wanted) {
    int err, errType;
